...
namespace PlayNext.Model.Score
{
	internal class TotalScoreCalculator
	{
		...
		private readonly RandomScoreCalculator _randomScoreCalculator = new RandomScoreCalculator();
		...
		public TotalScoreCalculator(PlayNext plugin)
		{
			...
			_finalGameScoreCalculator = new FinalGameScoreCalculator(plugin.HowLongToBeatExtension, _gameScoreByAttributeCalculator, _gameScoreBySeriesCalculator, _criticScoreCalculator, _communityScoreCalculator, _releaseYearCalculator, _gameLengthCalculator, _randomScoreCalculator, _scoreNormalizer, _summator);
		}
		...
		public ICollection<GameToPlayViewModel> Calculate(PlayNextSettings savedSettings)
		{
			...
			var gameScoreCalculationWeights = new GameScoreWeights()
			{
				...
				Random = savedSettings.RandomWeight / PlayNextSettings.MaxWeightValue,
			};
