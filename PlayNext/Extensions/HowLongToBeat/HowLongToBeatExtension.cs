...
using PlayNext.Extensions.HowLongToBeat.Data;
using Playnite.SDK;
...
namespace PlayNext.Extensions.HowLongToBeat
{
	public class HowLongToBeatExtension : IHowLongToBeatExtension
	{
		...
		private ConcurrentDictionary<Guid, int> _hltbData = new ConcurrentDictionary<Guid, int>();
		...
		public async Task ParseFiles(IEnumerable<Game> games)
		{
			try
			{
				...
				foreach (var data in datas.Where(x => x != null)
							 .ToDictionary(x => x.Id, x =>
								 x.Items.Sum(i => AverageTimes(i.GameHltbData)) / x.Items.Count)
							 .Where(x => x.Value > 0)
							 .ToDictionary(x => x.Key, x => x.Value))
				{
					_hltbData.AddOrUpdate(data.Key, data.Value, (x, y) => data.Value);
				}

				_logger.Info($"{_hltbData.Count} games with HowLongToBeat data stored");
			}
			catch (Exception ex)
			{
		...
		{
			return _hltbData.ToDictionary(x => x.Key, x => x.Value);
		}
